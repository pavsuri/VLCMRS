...
class UserFormValuesHtmlGenerator
{
    public static function  htmlInput($field, $optionsData, $formValues)
    {
        ...
        switch ($fieldType) {
            case "textbox":
                $textbox = "<input type='".$fieldType."' name='".$fieldName."' value='".$userValue."' class='form-control' />";
                $output = self::divStructure($fieldLabel, $textbox);
                break;
            case "textarea":
                $textarea =  " <".$fieldType."  name='".$fieldName."' rows='1'  class='form-control'>".$userValue."</textarea>";
            ...
            case "selectbox":
                ...
                break;
            case "image":
                $image =  "<input type='file' name='$fieldName-file' class='form-control'><img width='123px' src='uploads/".$userValue."'>";
                $output = self::divStructure($fieldLabel, $image); //$output =  "<img width='123px' src='uploads/".$userValue."'>";
                break;
            case "checkbox":
    ...
    public static function getCheckboxHtml($fieldLabel, $fieldName, $optionsData, $uuid, $userValue)
    {
        ...
        //echo "<pre>";print_r($optionsData); exit;
        foreach($optionsData as $option) {
            ...
            $structure .= '<div class="chk">
                            ...
                            <input type="checkbox" name="'.$fieldName.'[]" value="'.$option->fieldValue.'" '.$status.'  id="'.$uuid.'">
                        </div>';
        ...
        foreach($optionsData as $option) {
            ...
            $structure .= '<div class="chk">
                            <label for="'.$option->uuid.'">
                                <input type="radio" value="'.$option->fieldValue.'" name="'.$fieldName.'" '.$status.' id="'.$option->uuid.'"> 
